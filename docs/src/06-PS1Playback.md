...
Adrenaline allows the user to execute official PS1 games and custom PS1 games and applications on PlayStation Vita or PSTV.
...
> [!CAUTION]
> An older alternative to `pop-fe` is `PSX2PSP`, it is a unmaintained project and with compatibility lower than `pop-fe`, more so on the case of VITA/PSTV. A game converted with `PSX2PSP` will show up in Adrenaline, and most likely run, but will be unable to use custom POPS configuration, as the tool don't add a required section in the outputted `PBP` file to do custom POPS configuration injection.
...
The patch to re-enable CDDA has been added into Adrenaline's `PopCorn` module (the custom PS1 driver) from the start of the project, meaning that it is not necessary to use the `cdda_enabler` plugin on Adrenaline.

## Custom POPS Configuration

The PS1 emulator for PSP/Vita (POPS) allows for custom configuration to be injected in the structure of the PBP, allowing per-game fixes. This is used by some titles officially released in the PSN version.
...
Adrenaline contains a patch to defeat `libcrypt` protection for known games without requiring to rebuild the `PBP`file.

This patch is applied automatically on games detected to need it and does not require any configuration from the user.
