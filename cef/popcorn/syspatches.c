...
void PatchScePopsMgr(void) {
	...
	sctrlHookImportByNID(mod, "sceMeAudio", 0x2AB4FE43, sceMeAudio_2AB4FE43_Patched, 1);

	sctrlHookImportByNID(mod, "IoFileMgrForKernel", 0x6A638D83, sceIoReadPatched, 0);

	if (!g_is_official) {
		// Fake dnas drm
		sctrlHookImportByNID(mod, "IoFileMgrForKernel", 0x109F50BC, sceIoOpenPatched, 0);
		sctrlHookImportByNID(mod, "IoFileMgrForKernel", 0x63632449, sceIoIoctlPatched, 0);
...
void PatchPops(SceModule *mod) {
	...
	if (!g_is_official) {
		...
		sctrlHookImportByNID(mod, "scePopsMan", 0x0090B2C8, scePopsManExitVSHKernelPatched, 0);
