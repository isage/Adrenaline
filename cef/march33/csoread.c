...
static int inline ciso_get_index(u32 sector,int *pindex) {
	...
	if ((ciso_cur_index==0xffffffff) || (index_off<0) || (index_off>=CISO_INDEX_SIZE) ) {
		// out of area
		logmsg("%s: CISO READ INDEX sector=0x%08lX , loc=0x%08lX\n", __func__, sector,sizeof(ciso)+sector*4);
...
//	sceKernelDelayThread(20000);
	...
	logmsg("%s: CISO INDEX base 0x%08X offset 0x%08X\n", __func__,ciso_cur_index,index_off);
...
static int ciso_read_one(void *buf,int sector) {
	...
	logmsg("%s: CISO READ sector=0x%08X\n", __func__,sector);
	...
	if (index & 0x80000000) {
		// plain sector
		logmsg("%s: CISO plain read fpos=0x%08X\n", __func__,dpos);
		#ifdef SHORT_SLEEP
	...
	logmsg("%s: result sceKernelDeflateDecompress 0x%08X\n", __func__,result);
	if (result<0) {
		return result;
	}
