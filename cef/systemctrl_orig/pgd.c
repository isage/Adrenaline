...
int kirk7(u8 *buf, int size, int type) {
	...
	int retv = sceUtilsBufferCopyWithRange(buf, size + 0x14, buf, size, 7);
...
int get_version_key(u8 *version_key, char *path) {
	...
	sceDrmBBMacInit(&mkey, MAC_KEY_TYPE_FIXED);
	sceDrmBBMacUpdate(&mkey, header, 0xC0);
	ret = sceDrmBBMacFinal2(&mkey, header + 0xC0, version_key);
...
int get_edat_key(u8 *vkey, u8 *pgd_buf) {
	...
	if (pgd->drm_type == 1) {
		...
		if (pgd->key_index > 1) {
			pgd->mac_type = MAC_KEY_TYPE_FIXED;
			pgd_flag |= 8;
		}
	} else {
		pgd->mac_type = MAC_KEY_TYPE_FUSE_ID;
	}
	...
	sceDrmBBMacUpdate(&mkey, pgd_buf, 0x80);
	if (sceDrmBBMacFinal2(&mkey, pgd_buf + 0x80, fkey)) {
		return -2;
	}
